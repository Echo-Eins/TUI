...
use crate::app::{
    state::{MonitorStatus, ProcessSortColumn},
    App,
};
...
pub fn render(f: &mut Frame, area: Rect, app: &App) {
    let process_state = app.state.process_data.read();

    if let (MonitorStatus::Ready, Some(data)) = (&process_state.status, process_state.data.as_ref())
    {
