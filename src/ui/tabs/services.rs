...
use crate::app::{App, state::{ServiceSortColumn, ServiceStatusFilter}};
...
pub fn render(f: &mut Frame, area: Rect, app: &App) {
    ...
    if let Some(data) = service_data.as_ref() {
...
fn render_full(f: &mut Frame, area: Rect, data: &crate::monitors::ServiceData, app: &App, theme: &Theme) {
...
fn render_compact(f: &mut Frame, area: Rect, data: &crate::monitors::ServiceData, app: &App, theme: &Theme) {
    let chunks = Layout::default()
        .direction(Direction::Vertical)
        .constraints([
...
fn render_header(f: &mut Frame, area: Rect, data: &crate::monitors::ServiceData, app: &App, theme: &Theme) {
    let total_services = data.services.len();
    let running_services = data.services.iter().filter(|s| s.status == ServiceStatus::Running).count();
    let stopped_services = data.services.iter().filter(|s| s.status == ServiceStatus::Stopped).count();
    ...
    let header_text = vec![
        Line::from(vec![
            ...
            Span::styled(format!("{}", total_services), Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD)),
            Span::raw("  "),
            Span::styled("Running: ", Style::default().fg(Color::Gray)),
            Span::styled(format!("{}", running_services), Style::default().fg(Color::Green).add_modifier(Modifier::BOLD)),
            Span::raw("  "),
            Span::styled("Stopped: ", Style::default().fg(Color::Gray)),
            Span::styled(format!("{}", stopped_services), Style::default().fg(Color::Red).add_modifier(Modifier::BOLD)),
            Span::raw("  "),
            Span::styled("Filter: ", Style::default().fg(Color::Gray)),
            Span::styled(filter_text, Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD)),
        ]),
    ];
...
fn render_service_table(f: &mut Frame, area: Rect, data: &crate::monitors::ServiceData, app: &App, theme: &Theme) {
    ...
    // Apply sorting
    sort_services(&mut services, app.state.services_state.sort_column, app.state.services_state.sort_ascending);

    // Create table header with sort indicators
    let sort_indicator = if app.state.services_state.sort_ascending { "↑" } else { "↓" };

    let headers = vec![
        Cell::from(if app.state.services_state.sort_column == ServiceSortColumn::Name {
            format!("Name {}", sort_indicator)
        } else {
            "Name".to_string()
        }).style(Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD)),

        Cell::from(if app.state.services_state.sort_column == ServiceSortColumn::DisplayName {
            format!("Display Name {}", sort_indicator)
        } else {
            "Display Name".to_string()
        }).style(Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD)),

        Cell::from(if app.state.services_state.sort_column == ServiceSortColumn::Status {
            format!("Status {}", sort_indicator)
        } else {
            "Status".to_string()
        }).style(Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD)),

        Cell::from(if app.state.services_state.sort_column == ServiceSortColumn::StartType {
...
fn render_details_panel(f: &mut Frame, area: Rect, data: &crate::monitors::ServiceData, app: &App, theme: &Theme) {
    ...
    if let Some(service) = services.get(app.state.services_state.selected_index) {
        ...
        details.push(Line::from(vec![
            Span::styled("Service Details", Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD)),
        ]));
        ...
        details.push(Line::from(vec![
            ...
            Span::styled(&service.name, Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD)),
        ]));
        ...
        details.push(Line::from(vec![
            ...
            Span::styled(service.status.as_str(), Style::default().fg(status_color).add_modifier(Modifier::BOLD)),
            Span::raw("  "),
            Span::styled("Start Type: ", Style::default().fg(Color::Gray)),
            Span::styled(service.start_type.as_str(), Style::default().fg(Color::Cyan)),
        ]));
        ...
        details.push(Line::from(vec![
            ...
            Span::styled(
                ...
                Style::default().fg(if service.can_stop { Color::Green } else { Color::Red })
            ),
            Span::raw("  "),
            Span::styled("Can Pause: ", Style::default().fg(Color::Gray)),
            Span::styled(
                if service.can_pause_and_continue { "Yes" } else { "No" },
                Style::default().fg(if service.can_pause_and_continue { Color::Green } else { Color::Red })
            ),
        ]));
        ...
        if !service.dependent_services.is_empty() {
            details.push(Line::from(""));
            details.push(Line::from(vec![
                Span::styled("Dependent Services:", Style::default().fg(Color::Gray)),
            ]));
            details.push(Line::from(vec![
                Span::styled(service.dependent_services.join(", "), Style::default().fg(Color::White)),
            ]));
        }
