...
mod imp {
    ...
    impl ScreenCapturer {
        ...
        pub fn capture_frame(&mut self) -> Result<Option<CapturedFrame>, CaptureError> {
            ...
            let resized =
                dynamic.resize_exact(self.target_width, self.target_height, FilterType::Triangle);
...
mod imp {
    ...
    impl ScreenCapturer {
        ...
        pub fn capture_frame(&mut self) -> Result<Option<CapturedFrame>, CaptureError> {
            ...
            Ok(Some(CapturedFrame {
                sequence: self.frame_counter.fetch_add(1, Ordering::Relaxed),
                jpeg_data,
                width: self.target_width,
                height: self.target_height,
            }))
